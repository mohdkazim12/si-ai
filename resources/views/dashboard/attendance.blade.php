    ...
    <main class="flex-1 overflow-y-auto bg-gray-50 p-4 custom-scrollbar">
        <div class="max-w-7xl mx-auto">
                ...
                <ol class="inline-flex items-center space-x-1 md:space-x-3">
                    ...
                    <li>
                        <div class="flex items-center">
                            ...
                            <span class="text-sm font-medium text-primary">Users Attendance</span>
                        </div>
                    </li>
                </ol>
            </nav>
            ...
            <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-6">
                <div>
                    <h1 class="text-2xl font-bold text-gray-900">Users Attendance</h1>
                    <p class="text-gray-600">Manage all your Users Attendance in one place</p>
                </div>
        ...
        <div class="container mx-auto p-6">
            ...
            <div class="bg-white rounded-lg shadow-sm overflow-hidden border border-gray-100 mb-6">
                <div class="p-4 border-b border-gray-200 flex justify-between items-center">
                    <h2 class="text-lg font-medium text-gray-900">All Users Attendance<span class="text-gray-300">(Teacher and student Both)</span><span id="totalStudents" class="text-gray-500 text-sm"></span></h2>
                    <div class="flex items-center">
...
// <!-- JavaScript for Search and Pagination -->
    ...
    async function fetchData(page = 1, search = '' , per_page =  10) {
        try {
            const response = await fetch(`/attendance/details?page=${page}&search=${encodeURIComponent(search)}&per_page=${per_page}`);
            ...
            data.data.forEach(user => {
                const statusColor = user.attendance.status === 'Present' ? 'bg-green-100 text-green-800' : 
                                  'bg-red-100 text-red-800';

                const isAttendanceMarked = user.attendance.is_marked ? 'checked' : "no" ;
                const isAttendanceDisabled = user.attendance.is_marked ? 'disabled' : "no" ;
                const attendanceStatus = user.attendance.status ? user.attendance.status : "Absent" ;
                
                // console.log(isAttendanceMarked);
                // console.log(user.attendance_details);
                const row = `
                    <tr class="hover:bg-gray-50 cursor-pointer" data-student-id="${user.id}">
                        <td class="px-6 py-4 whitespace-nowrap">
                            <label class="custom-checkbox">
                                <input type="checkbox" class="custom-checkbox-input student-checkbox" id="attendance_checkbox" value="${user.id}" ${isAttendanceMarked} ${isAttendanceDisabled}>
    ...
    document.getElementById('searchInput').addEventListener('input', debounce((e) => {
        ...
        async function fetchDataWithFilters(page = 1) {
            ...
            try {
                const response = await fetch(`/attendance/details?${query}`);
                const data = await response.json();
        ...
        $(document).on('change', '.student-checkbox', function () {
                ...
                // Optional: Disable checkbox during request
                $(this).prop('disabled', true);

                $.ajax({
                    url: '/attendance/mark', // 👈 Your API route here
                    method: 'POST',
                    data: {
                        user_id: userId,
                        // status: isChecked ? '' : 0  // 👈 1 for present, 0 for absent/unmarked
                    },
                    headers: {
                        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content') // 👈 For Laravel or CSRF-protected routes
                    },
                    success: function (response) {
                        Swal.fire({
                        icon: 'success',
                    ...
                    error: function (xhr) {
                        ...
                        Swal.fire({
                            ...
                            text: 'Attendance was already recorded for today',
                            showConfirmButton: true,
                            confirmButtonColor: '#3B82F6',
